...
function AuthScreen({onLogin}){
  ...
  const handleLogin=()=>{
    ...
    signInWithEmailAndPassword(auth,email,pass)
      ...
      .catch(()=>{setErr("メールアドレスまたはパスワードが違います");})
  ...
  const handleRegister=()=>{
    ...
    createUserWithEmailAndPassword(auth,email,pass)
      ...
      .catch(()=>{setErr("このメールアドレスはすでに使われています");})
      .finally(()=>setLoading(false));
  };
...
export default function App(){
  ...
  useEffect(()=>{
    if(!user||isGuest){setIsAdmin(false);return;}
  ...
  useEffect(()=>{
    const unsub=onSnapshot(doc(db,"packs","pack1"),(d)=>{
      if(d.exists())setRemainingMap(prev=>({...prev,1:d.data().remaining}));
    });
    return()=>unsub();
  },[]);
  ...
  const doDraw=(pack)=>requireLogin(()=>{
    ...
    if(!isGuest&&user)setDoc(doc(db,"users",user.id),{coins:coins-pack.price,totalIssued:Math.max(0,totalIssued-pack.price)},{merge:true});
  ...
  const doMultiDraw=(pack,count)=>requireLogin(()=>{
    ...
    if(!isGuest&&user)setDoc(doc(db,"users",user.id),{coins:coins-totalCost,totalIssued:Math.max(0,totalIssued-totalCost)},{merge:true});
